...
from tuya2ildevice.tuya.runtime import BUILDERS, HostEnv, classify
ENV = default_env()
...
def run(platform):
    ...
    for code in fixtures.all_codes():
        ...
        for k in want.keys() - got.keys(): miss.append((code, k))
        for k in got.keys() - want.keys(): extra.append((code, k))
        for k in want.keys() & got.keys():
            w, g = want[k], got[k].identity
            for gk, ik in ATTRS:
                if w[gk] != g.get(ik): attr.append((code, k, gk, w[gk], g.get(ik)))
            cap = w["capabilities"] or {}
            if platform in ("number", "sensor"):
                eu = g.get("suggested_unit") or g.get("native_unit")
                if w["unit"] != eu and (code, k) not in HOST_DISPLAY_UNIT: attr.append((code, k, "unit", w["unit"], eu))
                if platform == "sensor" and w["device_class"] != g.get("device_class"): pass
            if platform == "sensor":
                if cap.get("state_class") != g.get("state_class"): attr.append((code, k, "state_class", cap.get("state_class"), g.get("state_class")))
                if cap.get("options") != g.get("options"): attr.append((code, k, "options", cap.get("options"), g.get("options")))
            if platform == "number":
                for ck, ik in (("min","native_min_value"),("max","native_max_value"),("step","native_step")):
                    if cap.get(ck) != g.get(ik): attr.append((code, k, "cap."+ck, cap.get(ck), g.get(ik)))
            st = schema_of(code)
            rd = got[k].read(st.status) if got[k].read else None
            if rd is not None and platform == "sensor":
                exp, val = w["state"], rd["native_value"]
                if not st.online: val = "unavailable"
                elif val is None: val = "unknown"
                conv = g.get("suggested_unit") not in (None, g.get("native_unit")) or (code, k) in HOST_DISPLAY_UNIT
                if not conv and exp != "unavailable":
                    try: same = abs(float(exp) - float(val)) < 1e-6 * max(1, abs(float(exp)))
                    except (TypeError, ValueError): same = exp == val
                    if not same and k not in ("wind_direction",): attr.append((code, k, "state", exp, val))
            if rd is not None and platform in STATE_OF:
                exp = w["state"]; val = STATE_OF[platform](rd)
                if not st.online: val = "unavailable"
                if val != exp: attr.append((code, k, "state", exp, val))
            if platform == "cover":
                if int(w["supported_features"]) != g["supported_features"]: attr.append((code, k, "features", w["supported_features"], g["supported_features"]))
                a = w["attributes"] or {}
                for ak, rk in (("current_position", "current_position"), ("current_tilt_position", "current_tilt_position")):
                    if a.get(ak) != rd[rk] and st.online: attr.append((code, k, ak, a.get(ak), rd[rk]))
            if platform == "humidifier":
                a = w["attributes"] or {}
                if int(w["supported_features"]) != g["supported_features"]: attr.append((code, k, "features", w["supported_features"], g["supported_features"]))
                for ck, ik in (("min_humidity", "min_humidity"), ("max_humidity", "max_humidity")):
                    if cap.get(ck) != g[ik]: attr.append((code, k, ck, cap.get(ck), g[ik]))
                if cap.get("available_modes") != g.get("available_modes") and "available_modes" in cap: attr.append((code, k, "modes", cap.get("available_modes"), g.get("available_modes")))
                if w["state"] not in ("unavailable",):
                    for ak, rk in (("humidity", "target_humidity"), ("current_humidity", "current_humidity"), ("mode", "mode")):
                        if a.get(ak) != rd[rk]: attr.append((code, k, ak, a.get(ak), rd[rk]))
            ...
            if platform == "vacuum":
                a = w["attributes"] or {}
                if int(w["supported_features"]) != g["supported_features"]: attr.append((code, k, "features", w["supported_features"], g["supported_features"]))
                if a.get("fan_speed_list", []) != g["fan_speed_list"] and "fan_speed_list" in a: attr.append((code, k, "fan_speed_list", a.get("fan_speed_list"), g["fan_speed_list"]))
                if a.get("fan_speed") != rd["fan_speed"]: attr.append((code, k, "fan_speed", a.get("fan_speed"), rd["fan_speed"]))
            if platform == "climate":
                a = w["attributes"] or {}
                if int(w["supported_features"]) != g["supported_features"]: attr.append((code, k, "features", w["supported_features"], g["supported_features"]))
                for ck, ik in (("hvac_modes", "hvac_modes"), ("fan_modes", "fan_modes"), ("preset_modes", "preset_modes"), ("swing_modes", "swing_modes")):
                    if a.get(ck) != g.get(ik): attr.append((code, k, ck, a.get(ck), g.get(ik)))
                if g["supported_features"] & 1:
                    for ck, ik in (("min_temp", "min_temp"), ("max_temp", "max_temp"), ("target_temp_step", "target_temperature_step")):
                        if cap.get(ck, a.get(ck)) != g[ik]: attr.append((code, k, ck, cap.get(ck, a.get(ck)), g[ik]))
                if w["state"] != "unavailable":
                    for ck, rk in (("current_temperature", "current_temperature"), ("temperature", "temperature"), ("current_humidity", "current_humidity"),
                        ...
                        if a.get(ck) != rd[rk]: attr.append((code, k, ck, a.get(ck), rd[rk]))
            if platform == "light":
                a = w["attributes"] or {}
                if sorted(cap.get("supported_color_modes", [])) != g["supported_color_modes"]: attr.append((code, k, "modes", cap.get("supported_color_modes"), g["supported_color_modes"]))
                if w["state"] == "on":
                    if a.get("color_mode") != rd["color_mode"]: attr.append((code, k, "color_mode", a.get("color_mode"), rd["color_mode"]))
                    ...
                    for ak in ("brightness",) + (("color_temp_kelvin",) if rd["color_mode"] == "color_temp" else ()):
                        if a.get(ak) != rd[ak]: attr.append((code, k, ak, a.get(ak), rd[ak]))
                    hs = rd["hs_color"]
            ...
            if platform == "fan":
                if int(w["supported_features"]) != g["supported_features"]: attr.append((code, k, "features", w["supported_features"], g["supported_features"]))
                a = w["attributes"] or {}
                if a.get("preset_modes") != g.get("preset_modes") and "preset_modes" in a: attr.append((code, k, "preset_modes", a.get("preset_modes"), g.get("preset_modes")))
                if w["state"] == "on":
                    for ak in ("percentage", "preset_mode", "oscillating", "direction"):
                        if ak in a and a[ak] != rd[ak]: attr.append((code, k, ak, a[ak], rd[ak]))
            if platform == "event" and cap.get("event_types") != g.get("event_types"): attr.append((code, k, "event_types", cap.get("event_types"), g.get("event_types")))
            if platform == "select":
                rd = got[k].read(schema_of(code).status)
                if cap.get("options") != rd["options"]: attr.append((code, k, "cap.options", cap.get("options"), rd["options"]))
...
def run_actions():
    ...
    for c in json.loads((pathlib.Path(__file__).parent / "golden" / "actions_golden.json").read_text()):
        ...
        if plat not in BUILDERS or act is None or "mock_device_code" not in c: continue
        ...
        for sk in ("fan_mode", "preset_mode", "swing_mode", "option"):    # HA service schemas coerce these to str (host)
            if sk in args: args[sk] = str(args[sk])
        ...
        if got != c["expected_commands"]:
            bad += 1; print("   ACTION MISMATCH", c["test"], c["entity_id"], got, c["expected_commands"])
